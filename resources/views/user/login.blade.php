                ...
                <div class="col-lg-8">
                    @if (!empty(request()->input('redirected')) && request()->input('redirected') == 'checkout')
                        <a href="{{ route('front.checkout', ['type' => 'guest']) }}"
                            class="btn btn-block btn-primary mb-4 base-btn py-3">{{ __('Checkout as Guest') }}</a>
                                ...
                                <div class="btn-group btn-group-toggle d-flex">
                                    @if ($be->is_facebook_login == 1)
                                        <a class="btn btn-primary text-white py-2 facebook-login-btn"
                                            href="{{ route('front.facebook.login') }}"><i
                                                class="fab fa-facebook-f mr-2"></i> {{ __('Login via Facebook') }}</a>
                                    @endif
                                    @if ($be->is_google_login == 1)
                                        <a class="btn btn-danger text-white py-2 google-login-btn"
                                            href="{{ route('front.google.login') }}"><i class="fab fa-google mr-2"></i>
                                            {{ __('Login via Google') }}</a>
                                    @endif
                                </div>
                            </div>
                        @endif
                            ...
                            <div class="input-btn mt-4">
                                ...
                                <div class="mt-2 d-flex justify-content-between">
                                    <a href="{{ route('user-register') }}"
                                        class="mr-3">{{ __("Don't have an account") }}?</a>
